...
public class RoombaController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        maxDistance = Vector2.Distance(transform.position, plug.transform.position);
    ...
    private bool IsMovementBlocked()
    {
        ...
        Vector2 cordDirection = plug.transform.position - transform.position;
        Vector2 driveDirection = transform.right;
        if (Vector2.Distance(transform.position, plug.transform.position) >= maxDistance - 1 && Vector2.Angle(driveDirection, cordDirection) >= 150)
            return true;
    ...
    IEnumerator RumbaWalk()
    {
        while (true)
        {
            ...
            while (timeDriven < driveTime && !IsMovementBlocked())
            {
